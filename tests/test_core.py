...
from mergefiles.core import (
    resolve_conflict_by_hash,
    keep_both_files,
    keep_files_from_preferred_src,
    collect_files_gen,
    merge_directories,
    copy_file
)
...
def test_copy_file():
    with TemporaryDirectory() as src, TemporaryDirectory() as dst:
        create_test_directories(src, {"file1.txt": "Hello"})
        src_file = Path(src) / "file1.txt"
        dst_file = Path(dst) / "file1.txt"

        summary = {'files_copied': 0, 'directories_created': 0, 'conflicts': [], 'errors': []}
        summary_lock = Lock()
        logger = setup_logger("test_logger")
        
        copy_file(str(src_file), str(dst_file), resolve_conflict_by_hash, summary, summary_lock, logger)
